...
export default function MemberEmailPage() {
  ...
  const handleSelectAdmins = () => {
    const adminIds = users.filter(user => user.role === 'admin').map(user => user._id);
    setSelectedUsers(adminIds);
  };

  const handleSelectMembers = () => {
    const memberIds = users.filter(user => user.role === 'member').map(user => user._id);
    ...
    <DashboardLayout requiredRole="member">
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-400">Takım arkadaşlarınıza e-posta gönderin</p>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-dark-card border border-dark-border rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <button
                  onClick={handleSelectAdmins}
                  className="text-xs bg-red-500/20 text-red-400 px-3 py-2 rounded-lg hover:bg-red-500/30 transition-colors"
                >
                  Adminleri Seç
                </button>
                <button
                  onClick={handleSelectMembers}
                  className="text-xs bg-blue-500/20 text-blue-400 px-3 py-2 rounded-lg hover:bg-blue-500/30 transition-colors"
                >
                  Üyeleri Seç
                </button>
              </div>
              ...
              <button
                ...
                className="w-full text-sm text-accent hover:text-accent/80 transition-colors mb-4 text-center"
              >
